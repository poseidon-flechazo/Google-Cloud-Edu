...
1. repositryを作成
```bash
gcloud artifacts repositories create　リポジトリ  --repository-format=docker --location=asia-northeast1
```
結果：
```
gcloud artifacts repositories create り --repository-format=docker --location=asia-northeast1
```
...
- tag
```bash
docker tag nginx:stable-alpine asia-northeast1-docker.pkg.dev/プロジェクト名/celery
```
- images
```bash
docker images
