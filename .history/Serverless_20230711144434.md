...
- tag
```bash
docker tag nginx:stable-alpine asia-northeast1-docker.pkg.dev/ca-qulijing-edu/celery
```
- images
```bash
docker images
...
- push
```bash
docker push asia-northeast1-docker.pkg.dev/ca-qulijing-edu/celery/nginx:stable-alpine
```
結果：
```
docker push asia-northeast1-docker.pkg.dev/ca-qulijing-edu/celery/nginx:stable-alpine
...
```
- id tokenを加えてリクエスト
    - `gcloud auth print-identity-tokenで取ってくる文字列はid token
