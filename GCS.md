...
- デフォルトのストレージクラス
- アクセス制御
`ca-qulijing-edu-test`という名前で東京リージョンと`Standard`ストレージクラスを作成
```bash
 gcloud storage buckets create gs://ca-qulijing-edu-test \
 --location=asia-northeast1 \
 --default-storage-class=Standard
```
結果：
```
Creating gs://ca-qulijing-edu-test/...
```

## オブジェクトのupdateとdownload
...
```bash
gcloud storage cp test gs://ca-qulijing-edu-test/
```
結果：
```
Copying file://test to gs://ca-qulijing-edu-test/test
  Completed files 1/1 | 0B
```
3. stroage bucketsからdownload
```bash
gcloud storage cp gs://ca-qulijing-edu-test/test ./
```
結果：
```
...
更新完了後に再度確認
```
Copying gs://ca-qulijing-edu-test/test to file://./test
...
4. 一覧
```bash
gcloud storage ls -a gs://ca-qulijing-edu-test/test
```
結果：
```
gs://ca-qulijing-edu-test/test#1688431696660060
gs://ca-qulijing-edu-test/test#1688432431213768
...
1. versioningを有効にする
```bash
gcloud storage buckets update gs://ca-qulijing-edu-test --versioning
```
結果：
```
⠼Updating gs://ca-qulijing-edu-test/...
  Completed 1
```
...
3. 確認
```bash
gcloud storage cp gs://ca-qulijing-edu-test/test#1688432431213768 ./
...
Created [https://sqladmin.googleapis.com/sql/v1beta4/projects/ca-qulijing-edu/instances/postgresql15].
...
```bash
cloud_sql_proxy -instances=ca-qulijing-edu:asia-northeast1:postgresql15=tcp:5432
```
結果：
```
2023/07/04 15:54:31 Rlimits for file descriptors set to {Current = 8500, Max = 9223372036854775807}
2023/07/04 15:54:36 Listening on 127.0.0.1:5432 for ca-qulijing-edu:asia-northeast1:postgresql15
...
```
2023/07/04 17:21:42 New connection for "ca-qulijing-edu:asia-northeast1:postgresql15"
2023/07/04 17:21:42 refreshing ephemeral certificate for instance ca-qulijing-edu:asia-northeast1:postgresql15
2023/07/04 17:21:45 Scheduling refresh of ephemeral certificate in 54m58.522266s
2023/07/04 17:21:45 New connection for "ca-qulijing-edu:asia-northeast1:postgresql15"
